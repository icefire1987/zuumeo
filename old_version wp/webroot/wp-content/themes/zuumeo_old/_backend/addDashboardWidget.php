...
function addDashboardWidget($cpt, $lang="") {
	global $polylang;
	global $config_cpt;
		...
		foreach($my_languages AS $lang) {
			...
			while ( $loop->have_posts() ) : $loop->the_post();
				...
				if($c['orderby'] == "menu_order") {
					//$drag_table_col = '<td class="drag"><img src="'.getStylesheetDirectoryURI().'/_images/drag.gif" /></td>';
				}
				...
				if(isset($polylang->model->get_post_language($post_id)->slug)) {
				   	$post_lang = '<img src="'.POLYLANG_URL.'/flags/'.$polylang->model->get_post_language($post_id)->description.'.png" style="float:left; margin: 2px 5px 0px 0px;" />';
				}
					...
					if(!empty($children)) {
					    ...
					    foreach($children AS $child_key => $child_value) {
					    	...
					    	if(!empty($child_children)) {
					        	...
					        	$final_child_children = '
					        		<table id="" class="table_dashboard" cellpadding="0" cellspacing="0" border="0" style="width:100%; margin:3px 0px 4px 0px;">
							...
							if(isset($polylang->model->get_post_language($child_value->ID)->slug)) {
								$children_post_lang = '<img src="'.getStylesheetDirectoryURI().'/_images/_flags/'.$polylang->model->get_post_language($child_value->ID)->slug.'.png" style="height:14px; float:left; padding: 0px 7px 0px 0px;" />';
							}
	...
	echo '
		...
		<style>
	    	...
	    	#sortable-table .drag { 
	    		cursor:pointer; 
	    		padding-left:12px;
	    		background: transparent url('.getStylesheetDirectoryURI().'/_backend/_images/drag.gif) 3px 5px no-repeat;
	    	}
